...
obsSimProgram = '/'.join( (os.environ['OBSERVATIONSIMROOT'],
                           os.environ['OBSERVATIONSIMCONFIG'],
                           'obsSim.exe') )
dataSubselectorProgram = '/'.join( (os.environ['DATASUBSELECTORROOT'],
                                    os.environ['DATASUBSELECTORCONFIG'],
                                    'dataSubselector.exe') )
...
class TextPanel:
    def __init__(self, parent):
        self.parent = parent
        self.text = ""
        self.hscroll = AutoScrollbar(parent, orient='horizontal')
        self.hscroll.grid(row=1, sticky="ew")
        self.vscroll = AutoScrollbar(parent, orient='vertical')
        self.vscroll.grid(row=0, col=1, sticky="ns")
        self.msgBox = Tkinter.Listbox(parent, height=30, width=90,
                                      bg='white', exportselection=1,
                                      selectmode = 'extended',
                                      setgrid=1, fg='black',
                                      xscrollcommand=self.hscroll.set,
                                      yscrollcommand=self.vscroll.set)
        self.msgBox.grid(row=0, col=0, sticky="nesw")
        self.hscroll.configure(command=self.msgBox.xview)
        self.vscroll.configure(command=self.msgBox.yview)
        parent.rowconfigure(0, weight=1)
        parent.columnconfigure(0, weight=1)
        self.msgBox.bind_all('<Control-KeyPress-c>', self.Copy)
    def Copy(self, *event):
        if self.msgBox.size():
    ...
    def writeText(self, text):
        self.text += text
        self.msgBox.insert(Tkinter.END, text.rstrip())
        self.msgBox.see(Tkinter.END)
        self.msgBox.update_idletasks()
