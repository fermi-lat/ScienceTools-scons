...
if [ "x$syspfiles" = x ]; then
  syspfiles="$INST_DIR/syspfiles"
else
  syspfiles=`echo ":$syspfiles:" | sed "s%:$INST_DIR/syspfiles:%:%g" | sed "s%::*$%%"`
  syspfiles="$INST_DIR/syspfiles$syspfiles"
fi
...
if (!$?syspfiles) then
  set syspfiles "$INST_DIR/syspfiles"
else
  set syspfiles `echo ":$syspfiles:" | sed "s%:$INST_DIR/syspfiles:%:%g" | sed "s%::*$%%"`
  set syspfiles "$INST_DIR/syspfiles$syspfiles"
endif
...
def fillScript(scriptFile, env, wrapper, script, executable):
    ...
    if env.GetOption('supersede') != '.':
        finalScript = finalScript.replace('${REPLACE-BASEDIR}', env.Dir(env.GetOption('supersede')).abspath)
    else:
    ...
    #Setup ROOTSYS
    finalScript = finalScript.replace('${REPLACE-ROOTSYS}', env['ROOTSYS'])
...
def generate(env):
    ...
    def createWrapperEmitter(target, source, env):
        target = []
        for src in source:
            target.append(env['SCRIPTDIR'].File(os.path.basename(str(src.abspath))))
