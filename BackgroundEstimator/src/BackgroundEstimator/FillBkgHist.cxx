...
int BackgroundEstimator::FillBackgroundHist(string GRB_DIR, TH1F * hROI_Max, double RA_BURST, double DEC_BURST, short int type, int verbosity, TH1F* hROI_Min, TH1F * hCtsvsEnergy_Est) {
  ...
  if (!UsingDefaultBinning) {
        ...
        for (int ibin_old = 1;ibin_old<=Energy_Bins_datafiles;ibin_old++) {
             ...
             double CorrectionFactor=GimmeCorrectionFactor(ibin_old,MET);
             if (fabs(CorrectionFactor-1)>0.3) printf("%s: weird correction factor.. %f\n",__FUNCTION__,CorrectionFactor);

             BKG_old[ibin_old-1]= TOOLS::Integrate(hMap[ibin_old], L_BURST,B_BURST, 10)*GimmeCorrectionFactor(ibin_old,MET);
