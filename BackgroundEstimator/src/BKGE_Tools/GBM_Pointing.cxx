...
void TOOLS::GBM_AppendPointingPlots(string PlotsFile) {
  ...
  char name[1000];
  TH1F * hSolarAngle[14],*hPtGal_L[14],*hPtGal_B[14],*hZenithAngle[14];
  ...
  for (int idet=0;idet<14;idet++) { //loop over 14 GBM dets
      ...
      sprintf(name,"hGBMZenithAngle_%dvsTime",idet);
      ...
      for (int ib=1;ib<=bins;ib++) {
         ...
         float zangle=GetGBMDet_Angle(idet, PtRaZenith, PtDecZenith, PtRaz, PtDecz, PtRax, PtDecx);
         hZenithAngle[idet]->SetBinContent(ib,zangle);
      }
...
int TOOLS::PlotGBMDet_Angle(int nDet, float RA, float DEC, TH1F* h, string Plots_File){
  ...
  for (int i=1;i<=h->GetNbinsX();i++) {
       ...
       h->SetBinContent(i,GetGBMDet_Angle(nDet, RA, DEC, PtRaz, PtDecz, PtRax, PtDecx));
  }
  fPlots->Close();
  return 0;  
}
...
int TOOLS::PlotGBMDet_ZenithAngle(int nDet, TH1F* h, string Plots_File){
  ...
  for (int i=1;i<=h->GetNbinsX();i++) {
       ...
       h->SetBinContent(i,GetGBMDet_Angle(nDet, PtRaZenith, PtDecZenith, PtRaz, PtDecz, PtRax, PtDecx));
  }
  fPlots->Close();
  return 0;  
}
