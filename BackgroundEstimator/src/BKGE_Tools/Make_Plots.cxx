...
*/
 ...
 for (long jj = 1; jj < nrows && !status; jj++) {
     ...
     int ibinSTOP = hRAZenithvsTime.FindBin(STOP);
 ...
 if (!FT2SECONDS) {
   ...
   Interpolate(&hPtRazvsTime,true);
   Interpolate(&hLatitudevsTime);
   Interpolate(&hLongitudevsTime,true);
   Interpolate(&hPtRaxvsTime,true);
   Interpolate(&hPtDeczvsTime);
   Interpolate(&hPtDecxvsTime);
   Interpolate(&hRAZenithvsTime,true);
...
void Interpolate(TH1F* hist,bool aRA) {
 #ifdef DEBUG
