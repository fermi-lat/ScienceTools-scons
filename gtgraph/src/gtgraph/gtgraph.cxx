...
class GtGraphApp : public st_app::StApp {
  public:
    GtGraphApp();
    virtual ~GtGraphApp() throw();
    virtual void run();
    virtual void prompt();
    virtual void plot();
...
GtGraphApp::GtGraphApp(): m_hist_plot(0) {}

GtGraphApp::~GtGraphApp() throw() { delete m_hist_plot; }
...
void GtGraphApp::prompt() {
  st_app::AppParGroup & pars = getParGroup("gtgraph");
  pars.Prompt();
  pars.Save();
}

void GtGraphApp::plot() {
...
void GtGraphApp::readBins(const std::string & file, const std::string & table, const std::string & low_bin,
  const std::string & high_bin, std::vector<double> & low, std::vector<double> & high) const {
  using namespace st_graph;
...
void GtGraphApp::readData(const std::string & file, const std::string & table, const std::string & data_field,
  bool is_vector, std::vector<std::vector<double> > & data) const {
  using namespace st_graph;
...
st_app::StAppFactory<GtGraphApp> g_factory;
